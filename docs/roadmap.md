...
**Known gap against criterion 1, as of the Federal Register and Congress.gov adapter builds (2026-09):** this development environment's egress policy blocks direct requests to `federalregister.gov`, `api.congress.gov` (and every other `.gov` domain tested, plus `openstates.org`) from both shell tools and `WebFetch` — the same restriction the original research pass hit. Congress.gov has a second, independent reason no live call was possible: no `CONGRESS_API_KEY` is configured in this environment either. Both adapters were instead built and verified against each API's own official documentation/source code (Federal Register: `source-inventory.md` §1.1; Congress.gov: `source-inventory.md` §1.2) — a strong but not equivalent substitute for a live request. **A live smoke-test run against each real API, from an environment where it's reachable and with a real Congress.gov key, is a pre-production requirement before this criterion can be marked satisfied for either adapter** — tracked alongside the existing Open States/LegiScan ToS action items as something to close before Phase 1 goes to production, not before development continues.
