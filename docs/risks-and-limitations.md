...
This tool is explicitly designed to track *new* activity, not to be a complete historical archive (per the project brief). This has a specific, non-obvious consequence: **a policy that existed before the tool started monitoring a given source will never appear as "new,"** and a policy from a source not yet onboarded (most of the 45 non-pilot states, initially) simply isn't tracked at all — its absence from the dashboard means "not yet monitored," not "doesn't exist." This distinction needs to be visible to DfG staff (e.g., via the Source Health page showing exactly which jurisdictions/policy-types are currently covered) so "the tool shows nothing" is never misread as "nothing is happening."
